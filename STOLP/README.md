...
При уменьшении начальной выборки до нескольких эталонных объектов, ожидаемо увеличилась скорость работы алгоритма Парзеновского окна и уменьшилось количество неправильно классифицируемых объектов(с 6 до 4 при **h = 0.1**).

Код финально проверки качества:
...
for(i in 1:n){
    class = PW(et, 0.1, xl[i,1:2],FALSE)
    print(class)
    print(xl[i,3])
    if(class != c(xl[i,3])) err = err + 1
}
print(err)
```
